...
abstract class BaseFacade24 {
    ...
    protected function process($request, $operation, $jsonHelper) {
        ...
        $jsonResponse = RestClient24::sendRequest($this->config, $jsonRequest, $headers);
    ...
    private function buildHeaders($request) {
        $header = new RequestHeader24();
        $header->setAuthToken($this->config->getAuthToken());
        $header->setHmac(HmacUtils24::genarateHmac($this->config->getHmacSecret(), $request));
    ...
    private function buildRequest($requestData, $operation, $jsonHelper) {
        $paycorpRequest = new PaycorpRequest24();
